...
def banner(console):
    ...
    print(Colorate.Horizontal(Colors.rainbow, '              𝗦𝗛𝗔𝗥𝗜𝗡𝗚 𝗧𝗛𝗘 𝗔𝗖𝗖𝗘𝗦 𝗞𝗘𝗬 𝗜𝗦 𝗡𝗢𝗧 𝗔𝗟𝗟𝗢𝗪𝗘𝗗'))
...
def prompt_valid_value(prompt_text, name, password=False):
    if password:
        return inquirer.secret(message=prompt_text).execute()
    else:
        return inquirer.text(message=prompt_text, validate=lambda val: len(val) > 0).execute()

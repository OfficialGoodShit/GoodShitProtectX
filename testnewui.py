...
def type_animated(text, delay=0.02, style="bold green"):
    for char in text:
...
def hacker_intro():
    ...
    lines = [
        ...
        "[ACCESS GRANTED]",
        "",
        "Welcome to: EmritzVipV6 Terminal",
...
def type_animated(text, delay=0.03, style="bold white"):
    for char in text:
        console.print(char, style=style, end='', soft_wrap=True)
        time.sleep(delay)
    console.print("")
...
def banner(console):
    clear()
    brand_name = "EmritzVipV6 : Facebook Chan Ramirez : TG KrishMonarchX"
    console.print(Text(brand_name, style="bold red"))

    line = "▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌▌"
    type_animated(line, style="bold green")
    type_animated("  𝗣𝗟𝗘𝗔𝗦𝗘 𝗟𝗢𝗚 𝗢𝗨𝗧 𝗙𝗥𝗢𝗠 𝗖𝗣𝗠 𝗕𝗘𝗙𝗢𝗥𝗘 𝗨𝗦𝗜𝗡𝗚 𝗧𝗛𝗜𝗦 𝗧𝗢𝗢𝗟", style="bold yellow")
    type_animated("       𝗦𝗛𝗔𝗥𝗜𝗡𝗚 𝗧𝗛𝗘 𝗔𝗖𝗖𝗘𝗦 𝗞𝗘𝗬 𝗜𝗦 𝗡𝗢𝗧 𝗔𝗟𝗟𝗢𝗪𝗘𝗗", style="bold red")
    type_animated(line, style="bold green")
...
def load_player_data(cpm):
    ...
    if response.get('ok'):
        ...
        if all(k in data for k in ['floats', 'localID', 'money', 'coin']):
            
            console.print(Text("========", style="bold red") + 
                          Text(" [ ᴘʟᴀʏᴇʀ ᴅᴇᴛᴀɪʟꜱ ] ", style="bold yellow") + 
                          Text("========", style="bold red"))
            
            time.sleep(0.3)
            type_animated(f"   >> Name        : {data.get('Name', 'UNDEFINED')}", style="bold green")
            time.sleep(0.1)
            type_animated(f"   >> LocalID     : {data.get('localID', 'UNDEFINED')}", style="bold green")
            time.sleep(0.1)
            type_animated(f"   >> Money       : {data.get('money', 'UNDEFINED')}", style="bold yellow")
            time.sleep(0.1)
            type_animated(f"   >> Coins       : {data.get('coin', 'UNDEFINED')}", style="bold cyan")

        else:
            type_animated("! ERROR: New accounts must sign in to the game at least once (✘)", style="bold red")
            time.sleep(1)
    else:
        type_animated("! ERROR: Your login is not properly set (✘)", style="bold red")
        time.sleep(1)
        exit(1)
...
def load_key_data(cpm):
    ...
    header = Text("========", style="bold red")
    header.append(" [ 𝘼𝘾𝘾𝙀𝙎𝙎 𝙆𝙀𝙔 𝘿𝙀𝙏𝘼𝙄𝙇𝙎 ] ", style="bold yellow")
    header.append("========", style="bold red")
    console.print(header)

    time.sleep(0.3)
    type_animated(f"   >> Access Key  : {data.get('access_key', 'N/A')}", style="bold green")
    time.sleep(0.1)
    type_animated(f"   >> Telegram ID : {data.get('telegram_id', 'N/A')}", style="bold cyan")
    time.sleep(0.1)

    balance = "Unlimited" if data.get("is_unlimited") else str(data.get("coins", 0))
    type_animated(f"   >> Balance     : {balance}", style="bold yellow")
